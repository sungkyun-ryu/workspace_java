...
class Fruit4 {
	...
	public Fruit4(String name, int price, String exp) {
		this.name = name; this.price = price; this.expire = exp; 
	}
...
public class Test_실습3_7객체배열정렬검색_람다식 {
	...
	private static int binarySearch(Fruit4 [] arr, Fruit4 fruit, Comparator<Fruit4> comp) {
		Arrays.sort(arr,comp);		
		int b = 0 ; int f = arr.length;
		while(b <= f) {
			int m = (b+f)/2; 
			if (comp.compare(arr[m], fruit) == 0)
				return m; 
			else if (comp.compare(arr[m], fruit) < 0)
				b = m+1; 
			else 
				f = m-1; 			
		} return -1;
	...
	public static void main(String[] args) {
		...
		Arrays.sort(arr, (a, b) -> a.getPrice() - b.price); 
