...
public class Test_ch10_lotto당첨처리 {
	...
	public static void lotto_generator(int n) {
		...
		HashSet<HashSet<Integer>> lot = new HashSet<>(); //복권의 집합
		HashSet<Integer> lotto = null; //한장의 복권
		List<List<Integer>> al = new ArrayList<>(); //보너스번호까지
		/*
		...
		for (HashSet<Integer> eachLotto : lot) {
			 ...
			 */
			//구현할 부분 - ArrayList 사용(마지막 번호 구분) Done!
			ArrayList<Integer> arrayLotto = new ArrayList<> (eachLotto) ; 
			arrayLotto.sort(null);			
			
			al.add(arrayLotto); 
			for(int i=0; i < arrayLotto.size()-1; i++) {
				System.out.print(arrayLotto.get(i) + "  ");
				
			}
			System.out.println("+ 보너스번호: " + arrayLotto.get(6));
